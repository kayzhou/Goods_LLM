    ...
    for epoch in range(epochs):
        ...
        with tqdm(train_loader, desc=f'Epoch {epoch + 1}/{epochs}', unit='batch') as t:
            for batch_X, batch_y in t:
                optimizer.zero_grad()
                output = model(batch_X.unsqueeze(1).to(device))  # 添加一个维度并移动到GPU
        ...
        model.eval()
        with torch.no_grad():
            test_output = model(X_test.unsqueeze(1).to(device))  # 添加一个维度并移动到GPU
...
results_df.to_csv('/home/llm/liguanqun/毕设new/代码/gru/tf-evaluation_results.csv', index=False)
