...
for i in range(1, 5):
    ...
    # 初始化模型、损失函数和优化器
    model = LSTMClassifier(input_size=vocab_size, hidden_size=128, output_size=len(df[f'cate_{i}'].unique())).to('device')
...
results_df.to_csv('/home/llm/liguanqun/毕设new/代码/lstm/wv-evaluation_results.csv', index=False)
