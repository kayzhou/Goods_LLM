...
class Config:
    ...
    model_path = "/home/llm/liguanqun/model/albert-chinese-base"
    checkpoint_path = "/home/llm/liguanqun/model/albert-chinese-base/checkpoints/"
    result_file = "/home/llm/liguanqun/毕设new/代码/albert/results.csv"
    max_len = 32
    batch_size = 16
    epochs = 5
...
def train_and_test_model(config, level, data, train_data, test_data, tokenizer):
    # 定义模型和训练参数
    model = AutoModelForSequenceClassification.from_pretrained(config.model_path, num_labels=data[f'encoded_label_{level}'].nunique())
    model.to(device)
    ...
    # 准备数据加载器
    train_dataset = CustomDataset(train_data, tokenizer, max_len=32)
...
def main():
    ...
    # 将整个程序运行的总时间记录到文件中
    with open("/home/llm/liguanqun/毕设new/代码/albert/time.txt", "a") as f:
